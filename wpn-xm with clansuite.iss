...
begin
  ...
  if (DEBUG = true) and (FileExists(expandconstant('c:\serverpack\nginx.zip')) = false) then
  begin
    // re-create the serverpack folder
    if not DirExists(ExpandConstant('c:\serverpack')) then ForceDirectories(ExpandConstant('c:\serverpack'));
    // Initialize InnoTools Download Helper
    itd_init;
    // Change from a simple overall progress bar to the detailed download view
    itd_setoption('UI_DetailedMode', '1');
